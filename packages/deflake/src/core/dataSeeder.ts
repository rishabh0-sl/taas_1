...
export class DataSeeder {
  ...
  static async seedFromFixture(
    ...
    for (let attempt = 1; attempt <= retryAttempts; attempt++) {
      try {
        ...
        console.log(`❌ Seeding attempt ${attempt} failed: ${errorMessage}`);
        
        if (attempt === retryAttempts) {
          throw new Error(`Failed to seed data after ${retryAttempts} attempts: ${errorMessage}`);
        }
  ...
  private static async seedUserViaAPI(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.log(`❌ Error seeding user ${user.email}: ${errorMessage}`);
    }
  }
  ...
  private static async seedProductViaAPI(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.log(`❌ Error seeding product ${product.name}: ${errorMessage}`);
    }
  }
  ...
  private static async seedOrderViaAPI(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.log(`❌ Error seeding order ${order.id}: ${errorMessage}`);
    }
  }
  ...
  static async resetTestData(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.log(`❌ Error resetting test data: ${errorMessage}`);
    }
  }
  ...
  static async getTestDataStatus(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.log(`❌ Error getting test data status: ${errorMessage}`);
      return null;
    }
  }
